...
//		public float JumpSpeed = 0.5f;
	...
	public void Revive()
	{
		AdManager.ShowVideoAd("192if3b93qo6991ed0",
			(bol) => {
				...
				{
					SetAdPanel(false);
					colliObj.GetComponent<Collider>().isTrigger = true;
					Destroy(colliObj);
					//DestroyImmediate(colliObj.gameObject);
					if (colliObj.name.Contains("Zombie") && (ShieldIcon || !isPlayerOnTheGround))
					{

						ZombieController zombieScript = colliObj.GetComponent<ZombieController>();
						zombieScript.DeathState();
						Ace_IngameUiControl.Static.inGameScoreCount++;
					}
					//for player hit Obsticles when shield is on
					else if (colliObj.tag.Contains("Obsticles") && ShieldIcon)
					{
						Destroy(colliObj);

					}
					//for player hit Rock when shield is off
					else if (colliObj.name.Contains("Rock") && ShieldIcon)
					{

						DestroyImmediate(colliObj);
					...
					else if (colliObj.name.Contains("broken_pot") || colliObj.name.Contains("broken_pot") && ShieldIcon)
					{
	...
	public void PlayerDie()
    {
		...
		Destroy(colliObj.GetComponent<Collider>());//for destroy zombie collider
